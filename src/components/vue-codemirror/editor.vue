  ...
  <div class="editor">
    <div class="main">
      <codemirror v-model="code" :style="{
        ...
        @update="handleStateUpdate" @ready="handleReady" @focus="log('focus', $event)" @blur="log('blur', $event)" />
      <pre v-if="preview" class="code"
        :style="{ height: config.height, width: preview ? '50%' : '0px' }">{{ code }}</pre>
    </div>
